...
router.route('/').get(getBlogs).post(upload.single('banner'), verifyToken, verifyRole('writer', 'admin'), validate(blogValidateSchema), postBlog)
...
router.route('/upload-image').post(verifyToken, upload.single('upload'), uploadImageController)
